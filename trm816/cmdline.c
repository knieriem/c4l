...
usage(void)
{
 ...
 [-p base] [-irq irq] [-d dnp] [-hw hw]\n\
    write data value to given io port\n\
    -p: portadress\n\
        1 char* value\n\
        default: `0x280'\n\
  -irq: Interrupt\n\
...
parseCmdline(int argc, char **argv)
{
  ...
  for(i=1, cmd.argc=1; i<argc; i++) {
    if( 0==strcmp("-p", argv[i]) ) {
      int keep = i;
      cmd.baseP = 1;
      i = getStringOpt(argc, argv, i, &cmd.base, 1);
      cmd.baseC = i-keep;
      continue;
    }

    if( 0==strcmp("-irq", argv[i]) ) {
      int keep = i;
      cmd.irqP = 1;
      i = getIntOpt(argc, argv, i, &cmd.irq, 1);
      cmd.irqC = i-keep;
      continue;
    }
    ...
    if( 0==strcmp("-d", argv[i]) ) {
      int keep = i;
      cmd.dnpP = 1;
      i = getStringOpt(argc, argv, i, &cmd.dnp, 1);
      cmd.dnpC = i-keep;
      continue;
    }

    if( 0==strcmp("-hw", argv[i]) ) {
      int keep = i;
      cmd.hwP = 1;
      i = getStringOpt(argc, argv, i, &cmd.hw, 1);
      cmd.hwC = i-keep;
      continue;
    }
